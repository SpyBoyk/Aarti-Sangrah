...
export default function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${
      ...
      {/* Premium Top Announcement Ribbon (#174478 Theme) */}
      <div className="relative overflow-hidden bg-gradient-to-r from-[#0e2c4e] via-[#174478] to-[#0e2c4e] px-3 py-1.5 text-center text-[11px] font-bold tracking-wider text-white shadow-xs sm:text-xs dark:from-stone-950 dark:via-[#0f345d] dark:to-stone-950 dark:text-sky-200">
      ...
      <div className="mx-auto flex max-w-7xl items-center gap-2.5 px-4 py-2.5 sm:gap-4 sm:px-6 md:px-8">
        ...
        <Link to="/" className="group flex min-w-0 items-center gap-2.5 sm:gap-3 focus:outline-none" aria-label="आरती संग्रह मुख्यपृष्ठ">
          ...
          <span className="leading-tight">
            ...
            <span className="hidden text-[9px] font-extrabold tracking-[0.2em] text-[#174478] uppercase sm:block dark:text-sky-300">
              मालघर • वाजेवाडी
            </span>
          </span>
        </Link>
          ...
          {links.map(({ to, label, icon: Icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `relative inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-xs font-extrabold transition-all duration-200 ${
                  isActive
                    ? 'bg-[#174478] text-white shadow-md shadow-[#174478]/25 dark:bg-sky-500 dark:text-stone-950'
            ...
            >
              ...
              {to === '/favorites' && favorites.length > 0 && (
                <span className="ml-0.5 rounded-full bg-amber-500 px-1.5 py-0.2 text-[10px] font-black text-stone-950 shadow-xs">
                  {favorites.length}
                </span>
              )}
            </NavLink>
        ...
        <form onSubmit={submit} className="ml-auto hidden min-w-0 flex-1 max-w-xs items-center md:flex" role="search">
          ...
          <div className="relative w-full">
            <Search size={14} className="absolute top-1/2 left-3.5 -translate-y-1/2 text-[#174478] dark:text-sky-400" />
            <input
              id="site-search"
              value={q}
              onChange={(e) => setQ(e.target.value)}
              placeholder="शोधा — गणेश, शिव, हनुमान…"
              className="w-full rounded-full border border-stone-200 bg-stone-50/80 py-2 pr-4 pl-9 text-xs font-semibold text-stone-900 placeholder:text-stone-400 transition focus:border-[#174478] focus:bg-white focus:ring-2 focus:ring-[#174478]/20 focus:outline-none dark:border-stone-700 dark:bg-stone-900/80 dark:text-stone-100 dark:focus:border-sky-400 dark:focus:ring-sky-400/20"
            />
          </div>
        </form>
        ...
        <button
          ...
          className="ml-auto grid h-9 w-9 shrink-0 place-items-center rounded-full border border-stone-200/80 bg-stone-50/50 text-[#174478] transition hover:bg-stone-100 active:scale-95 md:ml-0 dark:border-stone-700/80 dark:bg-stone-900/50 dark:text-amber-300 dark:hover:bg-stone-800"
        >
        ...
        <form onSubmit={submit} role="search">
          <div className="relative">
            ...
            <input
              ...
              enterKeyHint="search"
              className="w-full rounded-xl border border-stone-200 bg-stone-50 py-2 pr-4 pl-9 text-xs font-semibold text-stone-900 shadow-2xs focus:border-[#174478] focus:bg-white focus:outline-none dark:border-stone-700 dark:bg-stone-900 dark:text-stone-100"
            />
          </div>
        </form>
      </div>
          ...
          <nav className="grid gap-1" aria-label="मोबाईल नेव्हिगेशन">
            {links.map(({ to, label, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-xl px-4 py-2.5 text-xs font-extrabold transition active:scale-[0.99] ${
                    isActive
                      ? 'bg-[#174478] text-white shadow-sm dark:bg-sky-500 dark:text-stone-950'
              ...
              >
                ...
                {to === '/favorites' && favorites.length > 0 && (
                  <span className="ml-auto rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-black text-stone-950">{favorites.length}</span>
                )}
              </NavLink>
            ))}
            <NavLink
              to="/search"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 rounded-xl px-4 py-2.5 text-xs font-extrabold text-stone-700 hover:bg-stone-100 dark:text-stone-200 dark:hover:bg-stone-800/80"
            >
